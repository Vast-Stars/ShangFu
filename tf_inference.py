...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer12-fc4'):
        fc4_weights = tf.get_variable("weight", [Variables.FC3_SIZE, Variables.NUM_LABELS],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer is not None:
            tf.add_to_collection('losses', regularizer(fc4_weights))
        fc4_biases = tf.get_variable("bias", [Variables.NUM_LABELS], initializer=tf.constant_initializer(0.1))
